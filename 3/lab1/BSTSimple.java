...
public class BSTSimple {
	...
	class BST {
		...
		void insert(int x) {
			if (this.left == null && this.right == null && this.parent == null && this.key == 0) {
				this.key = x;
				return;
			}
			if (x > this.key) {
		...
		public BST pureNext(int x) {
			if (this.key == x) {
				return this;
			} else if (this.key < x) {
				if (this.right.key <= x) {
					return this.right.pureNext(x);
				} else {
					
				}
			} else {
				if ( TODO ) {}
				return this.left.pureNext(x);
			}
