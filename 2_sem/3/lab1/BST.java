...
public class BST {
	...
	public void solve() throws IOException {
		...
		RedBlackTree<Integer> set = new RedBlackTree<Integer>();
		...
		while (!"".equals(str = in.next())) {
			...
			case "insert":
				...
				break;
			case "delete":
				if (cunt != 0 && set.search(num) != null) {
					cunt--;
					set.remove(new RedBlackNode<Integer>(num));
				}
				break;	
			case "exists":
				if (set.search(num) == null || cunt == 0) {
					out.println("false");
				} else {
					out.println("true");
				}
				break;
			case "next":
				if (set.next(num) == null || cunt == 0) {
					out.println("none");
				} else {
					out.println(set.next(num).key);
				}
				break;
			case "prev":
				if (set.prev(num) == null || cunt == 0) {
					out.println("none");
				} else {
					out.println(set.prev(num).key);
				}
				break;
			default:
...
public class RedBlackTree<T extends Comparable<T>> {
	...
	public void remove(RedBlackNode<T> v){
		...
		if (!isNil(y.left))
			...
			x = y.right;
		// System.out.println(!isNil(y.left));
		// System.out.println(!isNil(y.right));
	...
	public RedBlackNode<T> next(T key) {
		if (isNil(pureNext(root, key))) {
			return null;
		}
		// System.out.println("Everybody in here!");
		return pureNext(root, key);
	}
	
	public RedBlackNode<T> prev(T key) {
		if (isNil(purePrev(root, key))) {
			return null;
		}
		return purePrev(root, key);
	}
	
	private RedBlackNode<T> pureNext(RedBlackNode<T> set, T key) {
			if (isNil(set)) {
				return nil;
			}
			// System.out.println("5th step" + " " + set.key + " " + set.color);
			if (set.key.compareTo(key) <= 0) {
				return pureNext(set.right, key);	
			} else {
				// System.out.println("10th step" + " " + set.key + " " + set.color);
				RedBlackNode<T> y = pureNext(set.left, key);
				if(isNil(y)) { 
					// System.out.println("20th step");					
					return set; 
				} else {
				 	return y;
				}
	...
	private RedBlackNode<T> purePrev(RedBlackNode<T> set, T key) {
			if (isNil(set)) {
				return nil;
			}
			// System.out.println("5th step" + " " + set.key + " " + set.color);
			if (set.key.compareTo(key) >= 0) {
				return purePrev(set.left, key);	
			} else {
				// System.out.println("10th step" + " " + set.key + " " + set.color);
				RedBlackNode<T> y = purePrev(set.right, key);
				if(isNil(y)) {
					// System.out.println("20th step");					
					return set; 
				} else {
